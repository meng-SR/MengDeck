...
const option = {
  ...
  tooltip: {},
  series: [
    //links 中是各个节点的连接关系，lineStyle可以配置连线的各个样式，color颜色、curveness线的曲度，这里要和配置流动点的lineStyle中的一致，否则，流动点会脱离
    //连线。width线条宽度
    {
      type: "graph",
      zlevel: 5,
    ...
    // data里面的coords是流动点，type=lines的数据项有三个，就可以达到三个数据点的流动效果,要注意其中effect中的period，其
    // 属性值越大，流动速度越快，effect中的color可以配置流动点的颜色
    {
      type: "lines",
...
export default {
  ...
  mounted: function() {
    const mychart = echart.init(document.querySelector(".flowChart"));
